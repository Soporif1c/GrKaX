...
Иконка в строке меню показывает состояние и даёт всё нужное без окна: подключить/отключить и переключение режима трафика. Закрытие окна **не разрывает соединение** — приложение сворачивается в меню-бар, полный выход тоже там.

В настройках можно включить запуск при входе в систему (обычный LaunchAgent в `~/Library/LaunchAgents`, без прав администратора) и автоподключение при старте. Ссылки `grkax://install-sub?url=…` работают так же, как на Android.

> **GrKa X не появится в «Системные настройки → VPN».** Эта запись создаётся только приложениями на Apple NetworkExtension, а разрешение на него Apple выдаёт лишь по платному Developer Program. Так же живут Clash Verge, V2rayU и sing-box. На работу это не влияет: трафик идёт через системный прокси или TUN.
